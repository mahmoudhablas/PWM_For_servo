...
//#define SYSCTL_RCC_PWMDIV_64     0x00000000  // /2
...
void PWM1A_Init(uint16_t period, uint16_t duty){
  ...
  GPIO_PORTB_PCTL_R &= ~0x00F0000;     // configure PB6 as PWM0
...
void PWM2A_Init(uint16_t period, uint16_t duty){
  ...
  GPIO_PORTE_AFSEL_R |= 0x20;           // enable alt funct on PE4
  GPIO_PORTE_PCTL_R &= ~0x0F00000;     // configure PE4 as PWM0
  GPIO_PORTE_PCTL_R |= 0x00400000;
  GPIO_PORTE_AMSEL_R &= ~0x20;          // disable analog functionality on PE4
  GPIO_PORTE_DEN_R |= 0x20;             // enable digital I/O on PE6
  SYSCTL_RCC_R = 0x00100000 |           // 3) use PWM divider
      (SYSCTL_RCC_R | (0x000E0000));   //    configure for /64 divider
  ...
  PWM0_ENABLE_R |= 0x00000020;          // enable P?4/M0PWM4
}
// change duty cycle of PB6
// duty is number of PWM clock cycles output is high  (2<=duty<=period-1)
void PWM2A_Duty(uint16_t duty){
  PWM0_2_CMPB_R = duty - 1;             // 6) count value when output rises
}
