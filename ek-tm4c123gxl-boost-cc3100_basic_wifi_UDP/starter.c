...
int main(void){
  ...
  ST7735_PlotClear(0,4095);  // range from 0 to 4095
	/*#############  Initialization of pins to act as pwm ###############*/
	PWM_Init_PF0(5000,250);   
	/*###################################################################*/
  while(1){
    sl_Start(0, 0, 0); /* Initializing the CC3100 device */
    /* Connecting to WLAN AP - Set with static parameters defined at the top
            ...
            UARTprintf("Receive error %d ",Status);
						...
						while(1)
						{
							...
							if( Status <= 0 ){
								...
								UINT8 *temp = uBuf+2;
								reading = (float)atof((char*)temp);
								o = map(reading);
								if(uBuf[0] == 'x' && uBuf[1] == '1' )
								{
									PWM_Duty_PF0(o);		
								}else if(uBuf[0] == 'x' && uBuf[1] == '2')
								{
									PWM_Duty_PF1(o);	
								}else if(uBuf[0] == 'x' && uBuf[1] == '3')
								{
									PWM_Duty_PF2(o);
								}
